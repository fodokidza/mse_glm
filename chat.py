...
that full candidate set -- SIX independent, summed vote layers:
...
Commands:
  ...
  /scores <prompt>          Open Mode only: full V1-V6 score breakdown for
...
def main():
    ...
    print("\n  Commands: /mode strict|open  /explain  /scores  /bigram  /shared  "
        ...
        if arg is not None:
            ...
            for c in sorted(info["scores"], key=info["scores"].get, reverse=True):
                ...
                print(f"    {c:13s} {v1:15.2f} {v2:15.2f} {v3:13.2f} {v4:15.2f} {v5:17.2f} "
                      f"{v6:14.2f} {info['scores'][c]:8.2f}{marker}")
